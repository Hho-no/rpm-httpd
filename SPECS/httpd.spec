...
%define mmn 20120211
#http://blog.endpoint.com/2009/08/defining-variables-for-rpmbuild.html
%define LIBS -lpthread
# https://www.redhat.com/archives/fedora-packaging/2007-March/msg00088.html

#https://docs.fedoraproject.org/ro/Fedora_Draft_Documentation/0.1/html/RPM_Guide/ch09s05s07.html
%define _unpackaged_files_terminate_build 0
%define _prefix /opt
%define _myconfdir %{_prefix}/etc
...
BuildRequires: apr-devel >= 1.4.0, apr-util-devel >= 1.4.0, pcre-devel >= 5.0, libnghttp2-devel >= 1.6
Requires: initscripts >= 8.36, /etc/mime.types, libnghttp2 >= 1.6
...
%configure \
	...
	--enable-layout=RPM \
	--libdir=%{_libdir} \
	--sysconfdir=%{_myconfdir}/httpd/conf \
...
ln -s %{_localstatedir}/log/httpd $RPM_BUILD_ROOT%{_myconfdir}/httpd/logs
ln -s %{_localstatedir}/run $RPM_BUILD_ROOT%{_myconfdir}/httpd/run
ln -s %{_libdir}/httpd/modules $RPM_BUILD_ROOT%{_myconfdir}/httpd/modules
mkdir -p $RPM_BUILD_ROOT%{_myconfdir}/httpd/conf.d

# install SYSV init stuff
mkdir -p $RPM_BUILD_ROOT/etc/rc.d/init.d
mkdir -p $RPM_BUILD_ROOT/etc/sysconfig

echo "HTTPD=/opt/sbin/httpd" > ./build/rpm/httpd.sysconfig
sed -i -e 's/\(User\|Group\) daemon/\1 apache/' $RPM_BUILD_ROOT/opt/etc/httpd/conf/httpd.conf
sed -i -e 's|CONFFILE=/etc/httpd/conf/httpd.conf|CONFFILE=/opt/etc/httpd/conf/httpd.conf|' ./build/rpm/httpd.init
...
install -m755 ./build/rpm/httpd.sysconfig \
	$RPM_BUILD_ROOT/etc/sysconfig/httpd
...
if [ ! -f %{_myconfdir}/httpd/conf/server.key ] ; then
%{_bindir}/openssl genrsa -rand /proc/apm:/proc/cpuinfo:/proc/dma:/proc/filesystems:/proc/interrupts:/proc/ioports:/proc/pci:/proc/rtc:/proc/uptime 1024 > %{_myconfdir}/httpd/conf/server.key 2> /dev/null
fi
...
if [ ! -f %{_myconfdir}/httpd/conf/server.crt ] ; then
cat << EOF | %{_bindir}/openssl req -new -key %{_myconfdir}/httpd/conf/server.key -x509 -days 365 -out %{_myconfdir}/httpd/conf/server.crt 2>/dev/null
--
SomeState
SomeCity
SomeOrganization
...
%dir %{_myconfdir}/httpd
%{_myconfdir}/httpd/modules
%{_myconfdir}/httpd/logs
%{_myconfdir}/httpd/run
%dir %{_myconfdir}/httpd/conf
%dir %{_myconfdir}/httpd/conf.d
%config(noreplace) /opt/etc/httpd/conf/httpd.conf
%config(noreplace) /opt/etc/httpd/conf/magic
%config(noreplace) /opt/etc/httpd/conf/mime.types
%config(noreplace) /opt/etc/httpd/conf/extra/httpd-autoindex.conf
%config(noreplace) /opt/etc/httpd/conf/extra/httpd-dav.conf
%config(noreplace) /opt/etc/httpd/conf/extra/httpd-default.conf
%config(noreplace) /opt/etc/httpd/conf/extra/httpd-info.conf
%config(noreplace) /opt/etc/httpd/conf/extra/httpd-languages.conf
%config(noreplace) /opt/etc/httpd/conf/extra/httpd-manual.conf
%config(noreplace) /opt/etc/httpd/conf/extra/httpd-mpm.conf
%config(noreplace) /opt/etc/httpd/conf/extra/httpd-multilang-errordoc.conf
%config(noreplace) /opt/etc/httpd/conf/extra/httpd-userdir.conf
%config(noreplace) /opt/etc/httpd/conf/extra/httpd-vhosts.conf
%config(noreplace) /opt/etc/httpd/conf/extra/proxy-html.conf
%config(noreplace) /opt/etc/httpd/conf/original/extra/httpd-autoindex.conf
%config(noreplace) /opt/etc/httpd/conf/original/extra/httpd-dav.conf
%config(noreplace) /opt/etc/httpd/conf/original/extra/httpd-default.conf
%config(noreplace) /opt/etc/httpd/conf/original/extra/httpd-info.conf
%config(noreplace) /opt/etc/httpd/conf/original/extra/httpd-languages.conf
%config(noreplace) /opt/etc/httpd/conf/original/extra/httpd-manual.conf
%config(noreplace) /opt/etc/httpd/conf/original/extra/httpd-mpm.conf
%config(noreplace) /opt/etc/httpd/conf/original/extra/httpd-multilang-errordoc.conf
%config(noreplace) /opt/etc/httpd/conf/original/extra/httpd-userdir.conf
%config(noreplace) /opt/etc/httpd/conf/original/extra/httpd-vhosts.conf
%config(noreplace) /opt/etc/httpd/conf/original/extra/proxy-html.conf
%config(noreplace) /opt/etc/httpd/conf/original/httpd.conf

%config %{_sysconfdir}/logrotate.d/httpd
%config %{_sysconfdir}/rc.d/init.d/httpd
%config %{_sysconfdir}/sysconfig/httpd
...
%config(noreplace) %{_myconfdir}/httpd/conf/original/extra/httpd-ssl.conf
%config(noreplace) %{_myconfdir}/httpd/conf/extra/httpd-ssl.conf
